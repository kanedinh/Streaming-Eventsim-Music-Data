...
def load_hdfs_to_postgres(spark, path: str, pg_table: str, pg_url: str, pg_properties: dict):
    '''
    Load data from HDFS to PostgreSQL

    Args:
        spark (SparkSession): A SparkSession object
        path (str): hdfs path
        pg_table (str): table's name
        pg_url (str): postgres path
        pg_properties (dict): postgres's properties
    
    Returns:
        write (spark.df.write): DataframeWriter
    '''
    # read data from HDFS
    try:
...
if __name__ == "__main__":
    ...
    # postgres (according container postgres)
